...
public class MapDao {
	...
	private static final String GET_ALL_TEAMS_SQL = "select id, name, ground, league, latitude, longitude, search_term, label_align from team order by id";
	private static final String GET_TEAMS_BY_LEAGUE_SQL = "select id, name, ground, league, latitude, longitude, search_term, label_align from team where league = ? order by id";
	...
	public List<Team> getAllTeams() {
		List<Team> teams = jdbcTemplate.query(GET_ALL_TEAMS_SQL, teamRowMapper);
		
		return teams;
	}
	
	public List<Team> getTeams(League league) {
		List<Team> teams = jdbcTemplate.query(GET_TEAMS_BY_LEAGUE_SQL, new String[]{league.getName()}, teamRowMapper);
